...
function rightSeekingAgent(): QLearningAgent {
  ...
  // Seed goal-conditioned keys: at cell x targeting checkpoint x+1, 'right' is best.
  for (let x = 0; x < 3; x++) {
    const target = { x: x + 1, y: 0 };
    agent.update(stateKey({ x, y: 0 }, target), 'right', 100, stateKey({ x: x + 1, y: 0 }, target));
  }
  return agent;
}
